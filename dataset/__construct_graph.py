...
def custom_knn_graph(x, k, batch=None, loop=False, flow='target_to_source', distance_type='euclidean'):
    """
    ...
    # Compute pairwise distances according to the specified distance type
    if distance_type == 'euclidean' or distance_type == 'minkowski':
        # Using scipy's cdist to compute the Minkowski distance (p-norm)
        p = 2 if distance_type == 'euclidean' else 3  # Example for cubic Minkowski
        dists = cdist(x.detach().numpy(), x.detach().numpy(), metric='minkowski', p=p)
    elif distance_type == 'chebyshev':
        dists = cdist(x.detach().numpy(), x.detach().numpy(), metric='chebyshev')
    elif distance_type == 'manhattan':
        dists = cdist(x.detach().numpy(), x.detach().numpy(), metric='cityblock')
    elif distance_type == 'cosine':
        dists = 1 - cosine_similarity(x.detach().numpy())  # 1 - similarity for distance
    elif distance_type == 'jaccard':
        dists = cdist(x.detach().numpy(), x.detach().numpy(), metric='jaccard')

    # Convert distance matrix to torch tensor
    dists = torch.from_numpy(dists).to(x.device)

    # Compute k-nearest indices from the distance matrix
    knn_indices = dists.argsort(dim=1)[:, 1:k+1]  # Exclude self-loop

    # Prepare edge indices for creating the graph
    row_indices = torch.arange(x.size(0), device=x.device).repeat_interleave(k)
...
def Knn_graph(data,edge_type,label,edge_norm,K):
    ...
    :return: knn graph
    '''
    # x1 = torch.tensor(data, dtype=torch.float)  # 节点特征
    data = (data.reshape(data.shape[0], -1) if len(data.shape) == 3 else data)  # 平铺
    x = torch.tensor(data, dtype=torch.float)  # 节点特征
    batch = torch.tensor(np.repeat(0,data.shape[0]), dtype=torch.int64)
    edge_index = custom_knn_graph(x, k=K, batch=batch, loop=False, distance_type="euclidean")  # K为邻居数，此处为有向图
...
def generate_sftf_graph(feature,graph_type,node_num,direction,edge_type,edge_norm,K,p):
    '''
    :param feature: shape (classes，sample_num，sample_length)  classes-故障类型数；sample_num-每种故障样本数；sample_length-每个样本长度
    :param graph_type: 图类型
    :param node_num: 每个图的节点个数
