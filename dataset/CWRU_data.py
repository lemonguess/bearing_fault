...
#内圈故障数据0.07英寸、0.14英寸、0.21英寸/负载0、1、2、3
...
#外圈故障数据0.07英寸、0.14英寸、0.21英寸/负载0、1、2、3 in Centerd @6:00
...
#滚动体故障数据0.07英寸、0.14英寸、0.21英寸/负载0、1、2、3
...
def data_preprocessing(dataset_path,sample_number,dir_path,window_size,overlap,normalization,noise,snr,
    ...
    if input_type == 'TD':  #时域信号
        ...
        _, _, magnitude = stft(all_data[0, :, :], nperseg=window_size, noverlap=overlap_samples)
        data = np.zeros((all_data.shape[0], all_data.shape[1], magnitude.shape[1], magnitude.shape[2]))
        for label_index in range(all_data.shape[0]):
            _, _, magnitude = stft(all_data[label_index,:,:], nperseg=window_size, noverlap=overlap_samples)
            # 提取频谱特征
    ...
    # 将剩余数据划分为测试集和验证集
    test_data, val_data = train_test_split(remaining_data, train_size=0.5)
