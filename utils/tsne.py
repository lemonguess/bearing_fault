...
def plot_embedding(data, label,classes):
    ...
    hand = list(map(lambda x: x[0] if not isinstance(x, int) else x, fig_leg))
    ...
    plt.savefig(r'E:\lemon\Documents\论文-李\GNN_FD-谭启瑜\GNN_FD-main\图神经网络故障诊断\output\{}.png'.format(uuid.uuid4().hex), dpi=600,
                bbox_inches='tight')
    return fig